...
**`git commit` 之前，必须对所有改动代码进行一轮完整自审核**，逐文件检查以下维度：

| 维度 | 检查内容 |
|------|---------|
| **代码正确性** | 逻辑是否正确，条件分支是否完备，边界情况是否处理 |
| **健壮性** | nil 兜底、空表/空字符串防御、除零保护、越界检查 |
| **Bug 排查** | 字段名拼写是否与源模块导出一致、变量作用域是否正确、循环索引是否从 1 开始 |
| **API 方法** | 调用的函数名是否存在、参数顺序是否正确 |
| **参数类型** | 传入参数类型是否匹配（number/string/table）、返回值是否正确使用 |
| **参数数量** | 函数调用参数个数是否与定义一致，不多不少 |
